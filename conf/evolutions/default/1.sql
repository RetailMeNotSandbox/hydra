...
CREATE TABLE resource_references (
  ...
  to_id JSONB NOT NULL,
  -- to_id can't be a foreign key reference to `resource`,
  -- since hydra allows references to resources that don't exist
...
WHEN (
  ...
  OR OLD.resource->'attributes' IS DISTINCT FROM NEW.resource->'attributes'
  OR OLD.resource->'relationships' IS DISTINCT FROM NEW.resource->'relationships'
)
EXECUTE PROCEDURE propagate_resource_edit();
...
-- ScratchExpander grabs from change_history_to_expand,
-- expands into incoming ids, and upserts all of them here
