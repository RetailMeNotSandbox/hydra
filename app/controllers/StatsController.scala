...
@Singleton
class StatsController @Inject() (@Named("subscription-manager") subMgr: ActorRef, @Named("expander-manager") expMgr: ActorRef)(implicit ec: ExecutionContext) extends Controller with Instrumented {
  ...
  def getBuild = Action.async {
    ...
    def response(status: Int, extraInfo: JsObject) : Result =
      Status(status)(buildInfo ++ extraInfo).withHeaders("Cache-Control" -> "must-revalidate,no-cache,no-store")
